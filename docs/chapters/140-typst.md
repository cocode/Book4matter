...
- **Print** takes the long way round. Pandoc converts the Markdown into a Typst
  *body fragment*, and Typst typesets that into the PDF.
- **EPUB and HTML** are written by Pandoc directly.

Why route print through Typst at all? Because Typst alone cannot produce EPUB,
and authoring straight in Typst would forfeit the EPUB and HTML outputs. With
Pandoc in the middle, all three come from the same Markdown. Typst earns its
place on the print side: it is fast, needs no enormous TeX installation, embeds
fonts automatically, and ships in the same `pandoc/typst` image.

### What a print build generates
...
Typst compiles `main.typ` into the interior PDF. Without `--keep`, these files
are removed after the build.

### The template

`book.typ` owns the print-critical geometry --- the exact trim, the asymmetric
binding gutter, font embedding, and page numbering --- and the styling: the title
and copyright pages, the table of contents, and the show rules that turn each
heading level into a part, a chapter, or a section. That styling is adapted from
the MIT-0-licensed *ilm* Typst template.
